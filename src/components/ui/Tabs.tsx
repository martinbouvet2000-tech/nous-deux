...
export default function Tabs<K extends string>({ tabs, value, onChange, label }: { tabs: Tab<K>[]; value: K; onChange: (k: K) => void; label: string }) {
  return (
    <div className="inline-flex w-full md:w-fit max-w-full overflow-x-auto gap-1 p-1 rounded-full bg-white/[0.04] shadow-[inset_0_0_0_1px_rgba(240,234,224,0.06)]" role="tablist" aria-label={label}>
      {tabs.map(({ key, label: l, icon: Icon }) => {
        ...
        return (
          <button
            ...
            className={`tap-44 flex-1 md:flex-none min-h-10 px-3 md:px-4 rounded-full text-[13px] font-medium whitespace-nowrap transition-all duration-200 flex items-center justify-center gap-1.5 ${
