...
export default function InstallAppButton() {
  ...
  const handleInstall = async () => {
    if (isIOS) {
      toast.info('Utilisez le partage pour ajouter a l ecran d accueil.')
      setShowModal(false)
      if (navigator.share) {
        navigator.share({
          title: 'Awy',
          text: 'Installez Awy sur votre ecran d accueil pour un acces rapide et securise.',
        }).catch(() => {})
      }
      return
    }
          ...
          >
            ...
            <div className='px-6 py-5 space-y-5'>
              <p className='text-sm text-[#F0EAE0]/90 leading-relaxed'>
                {isIOS
                  ? 'Ajoute Awy sur ton ecran d accueil pour un acces instantane. Appuie sur le bouton ci-dessous pour commencer.'
              ...
              {isIOS && (
                <div className='bg-white/[0.03] border border-white/[0.06] rounded-xl p-4 space-y-3'>
                  ...
                  <ol className='space-y-2 text-xs text-[#B8A793] leading-relaxed'>
                    <li><span className='text-[#F0EAE0] font-medium'>1.</span> Appuie sur le bouton partage (carre avec fleche)</li>
                    <li><span className='text-[#F0EAE0] font-medium'>2.</span> Fais defiler et selectionne Ajouter a l ecran d accueil</li>
