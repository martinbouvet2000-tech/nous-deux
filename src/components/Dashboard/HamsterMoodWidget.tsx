...
export default function HamsterMoodWidget({ className = '' }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    if (partnerProfile) {
      ch.on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'moods', filter: `user_id=eq.${partnerProfile.id}` }, (p) => {
        ...
        if (d) toast.info(`${partnerProfile.display_name} · ${d.label.toLowerCase()}`)
      })
    }
    ch.subscribe()
  ...
  const person = (name: string, def: MoodDef | null, row: Mood | null, isMe: boolean) => (
    <div className={`flex flex-col items-center justify-center text-center rounded-2xl px-3 pt-4 pb-3 ${def ? 'bg-white/[0.035]' : 'bg-white/[0.02]'} shadow-[inset_0_0_0_1px_rgba(240,234,224,0.07)]`}>
      ...
      <span className="text-[13px] leading-snug text-[#F0EAE0] font-medium">{def ? def.label : isMe ? 'Comment tu te sens ?' : 'En attente…'}</span>
    ...
    <section className={`lux-card relative overflow-hidden rounded-[20px] p-5 md:p-6 ${className}`} onMouseMove={shine} onMouseLeave={unshine} aria-labelledby="hamster-title">
      ...
      {picking ? (
        <div className="animate-fade-in">
          ...
          <div className="grid grid-cols-2 gap-2" role="group" aria-labelledby="hamster-pick-label">
            {MOODS.map((d) => {
